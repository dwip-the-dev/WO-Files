...
static void on_path_enter(GtkEntry *e){
    ...
    if(g_file_test(t, G_FILE_TEST_IS_DIR)) load_path(t, TRUE);
    else gtk_entry_set_text(GTK_ENTRY(path_entry), current_path);
}
...
static gboolean on_right(GtkWidget *w, GdkEventButton *ev){
    ...
    char *path=NULL; gboolean is_dir=FALSE;
...
static void add_shortcut(GtkButton *b){
    GtkWidget *d = gtk_file_chooser_dialog_new("Select Folder",NULL,
